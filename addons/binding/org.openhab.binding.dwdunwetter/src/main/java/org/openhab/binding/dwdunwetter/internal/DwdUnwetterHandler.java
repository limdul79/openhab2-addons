...
public class DwdUnwetterHandler extends BaseThingHandler {
    ...
    private void refresh() {
        ...
        int warningCount = getDwdConfig().getWarningCount();
        ...
        for (int i = 0; i < warningCount; i++) {
            ...
            if (warning == OnOffType.ON) {
                updateState(getChannelUuid(DwdUnwetterBindingConstants.CHANNEL_WARNING, i), warning);
            }
            if (data.isNew(i)) {
     ...
     */
    private DwdUnwetterConfiguration getDwdConfig() {
        if (config == null) {
    ...
    public void initialize() {
        ...
        data = new DwdWarningsData(getDwdConfig().getCellId());
    ...
    private Channel createTriggerChannel(String typeId, String label, int warningNumber) {
        ...
        return ChannelBuilder.create(channelUID, "String") //
                .withType(new ChannelTypeUID(DwdUnwetterBindingConstants.BINDING_ID, typeId)) //
    ...
    /**
     * Creates the Channel for each warning.
     *
    ...
    private void startAutomaticRefresh() {
        ...
        }, 0, getDwdConfig().getRefresh(), TimeUnit.MINUTES);
    }
